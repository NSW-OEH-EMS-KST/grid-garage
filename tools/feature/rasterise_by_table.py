...
class RasteriseByTableTool(BaseTool):
    ...
    def rasterise(self, data):
        ...
        for field in target_fields:
            r_out = "not set"
            try:
                r_out = base.utils.make_raster_name("{0}_{1}".format(splitext(feat_ds)[0], field), self.result.output_workspace, self.output_filename_prefix, self.output_filename_suffix)
